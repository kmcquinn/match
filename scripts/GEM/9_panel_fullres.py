...
'''
cmdfile_name = "12878_KDG215/out_vfm_3.cmd"
outfile_name = "12878_KDG215/out_vfm_3.final"
plot_name = "vfm_3.png"
...
def plot_Scatter(fig, x, y, subx1, suby2, subsize, title, xlabel, ylabel,linestyle, errlow=[], errhigh=[], bound=False,fill=True):    
      x = np.array(x)
      y = np.array(y)
      ...
      if len(errlow) == 0:
          ...
          if fill == True:
              ax.plot(x, y,'k', linestyle = linestyle)
              ax.fill_between(x, y - errlow, y + errhigh, facecolor='k',alpha=0.2)
          else:
...
ax_3 = plot_Scatter(fig, lage_arr1, met_arr1, subx2, suby1, subsize, '(h) Metallicity', 'log(age)','Z','solid', met_low1, met_up1, bound = False, fill = False)
