...
# i don't even think this code runs hybridMC, it literally just puts the command you want into the output files????? ugh why
...
##hmc_param_mp.py
##Finds dt, tint, and sscale parameters for ZINC runs of hybridMC.
...
def get_sig_frac(sresult):
    ...
    #stemp should be 3 for zinc runs I think 
    itemp = stemp[3]
    st=itemp.find('SD = ')+5
    end=itemp.rfind(')')
...
hconsolefs=[]
for i in datfs:
    mcmcfs.append(basepath+'hmc_param.mcmc')
    mcmcfs_out.append(basepath+'out.mcmc')
    hconsolefs.append(basepath+'hybrid_params.dat')
...
for i,e in enumerate(datfs):
    ...
    print out_sh_t
    print shlex.split(out_sh_t)
    p = subprocess.Popen(shlex.split(out_sh_t),stdout=subprocess.PIPE)
