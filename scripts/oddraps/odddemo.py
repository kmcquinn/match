...
import fnmatch
'''
OK so the current problem is that I thought we were able to run sbatch commands within a sbatch program.
i.e. this script is queued and run using sbatch and it generates more sbatchs to run calcsfh

but it seems like sbatch can only be run on a login node, which they don't like people using for extended time
...
#passed
	...
	#store filter values
	'''
	replace this with a fnmatch thing
	'''
...
def calcFit(bir, scr, filtStart):
	...
	runBatch(comm, "FirstCalc", "00:20:00")
	...
	while True:		#will stop loop 'manually' inside once end of cycle yield no positive change in lum
		...
		useLauncher("DepthRuns", commarr, "02:00:00")
		#now go back through console outputs to record fits for each run
		for i in range(0, flail):
			strflail = '%03d' % (i,)
			consolepath = scrstring+"consoleTEST"+strflail
			Tfitline = sp.check_output(['tail', '-1', consolepath]).split()[-1]
