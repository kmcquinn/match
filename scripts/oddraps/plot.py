...
#Below is the old plot_Hist, which I hated. The error bars were slightly offset
#in this graph??? It was weird. I'm leaving it in in case anyone ever wants it, 
...
"""def plot_Hist(fig, x, y, subx1, suby2, subsize, title, xlabel, ylabel,linestyle, errlow=[], errhigh=[], bound=False):
	...
	ax = fig.add_axes([subx1, suby2, subsize, subsize])
	ax.step(x, y, color='k')
	ax.errorbar(x,y,yerr=[errlow,errhigh], fmt='none', capsize=0, ecolor='r', elinewidth=0.7)
	ax.set_title(title, fontsize=8)
	ax.set_xlabel(xlabel, fontsize=8)
	ax.set_ylabel(ylabel, fontsize=8)
	if bound == True:
...
#here is a dumb way of reading in a text file. I am using a better way below
...
#I couldnt just change csf_arr bc there's some plots that don't need that 0
